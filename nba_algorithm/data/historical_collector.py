...
def fetch_historical_games(days: int = 30) -> List[Dict]:
    ...
    Raises:
        ValueError: If API key is missing or there's insufficient data
    ...
    try:
        ...
        # Fetch games with proper pagination handling
        all_games = []
        page = 1
        per_page = 100
        total_pages = 1
        ...
        while page <= total_pages and page <= 5:  # Limit to 5 pages to avoid rate limits
            try:
                ...
                if not response or not isinstance(response, dict):
                    logger.error(f"Invalid response format from API: {response}")
                    raise RuntimeError("Invalid response format from BallDontLie API")
                ...
                logger.error(f"Error fetching page {page} of historical games: {str(e)}")
                # Instead of continuing with partial data, raise the exception
                raise RuntimeError(f"Failed to fetch historical games: {str(e)}")
        
        logger.info(f"Successfully fetched {len(all_games)} historical games")
        
        # Verify we have meaningful data
        if not all_games:
            logger.warning("No historical games found in specified date range")
            return []
        ...
        for game in all_games:
            ...
            if not game.get("id") or not game.get("home_team") or not game.get("visitor_team"):
                logger.debug(f"Skipping game with missing critical data: {game.get('id')}")
                continue
            
            valid_games.append(game)
        
        logger.info(f"{len(valid_games)} valid games after filtering")
        ...
        logger.error(f"Unexpected error fetching historical games: {str(e)}")
        logger.debug(traceback.format_exc())
        raise RuntimeError(f"Failed to fetch historical NBA games: {str(e)}")
